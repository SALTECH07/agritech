// Alias for device firmware: GET /api/irrigation/decision?device_id=...
// Returns { allow_irrigation, forecast: { rain_probability_percent, rain_amount_mm }, reason }
...
export const Route = createFileRoute("/api/irrigation/decision")({
  server: {
    handlers: {
      ...
      GET: async ({ request }) => {
        ...
        // Prefer a recent snapshot (< 30 min). Otherwise fetch live weather
        // using device coordinates so the controller gets a real decision from boot.
        ...
        const isFresh =
          weather?.recorded_at && Date.now() - new Date(weather.recorded_at).getTime() < freshMs;
        ...
        const blockProb = Number(device.rain_block_probability ?? 70);
        const blockAmt = Number(device.rain_block_amount_mm ?? 5);
        const willRain = rainProb >= blockProb || rainAmt >= blockAmt;
        const allow = !willRain;
        const reason = willRain
